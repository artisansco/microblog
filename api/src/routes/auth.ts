...
authRouter.post("/register", async (c) => {
  try{
    ...
    const user = await db.select().from(usersTable).where(
      and(eq(usersTable.username, username),  eq(usersTable.email, email))
    ).limit(1)
...
authRouter.post("/login", async(c) => {
  try{
    ...
    const accessToken = await Jwt.sign({ id: user[0].id, username:user[0].username, exp: 24 * 60 * 60}, process.env.JWT_SECRET!, 'HS256')
